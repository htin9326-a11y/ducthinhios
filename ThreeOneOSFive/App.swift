...
struct ThreeOneOSFiveApp: App {
    ...
    init() {
        setupLogCapture()
        log("app: Dexter VN launching — iOS \(AppInfo.osVersion) (\(AppInfo.osBuild)) \(AppInfo.machineName)")
    }
...
private struct LicenseFailureOverlay: View {
    ...
    var body: some View {
        ZStack {
            ...
            RadialGradient(
                colors: [AppTheme.accent.opacity(pulse ? 0.28 : 0.10), Color.clear],
                center: .center,
                startRadius: 20,
                endRadius: 280
            )
                ...
                ScrollView {
                    VStack(spacing: 22) {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            featureLine("Dexter VN • Secure License", icon: "lock.shield.fill")
                        }
                        .font(.caption)
